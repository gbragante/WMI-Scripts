# WMIRPC-TraceParse - 20210813
# by Gianni Bragante - gbrag@microsoft.com

param (
  [string]$FileName = "C:\files\wmi-rpc-dcom-trace-wcpif333-!FMT.txt"
)
...
$col = New-Object system.Data.DataColumn SubjectPointer,([string]); $tbRPC.Columns.Add($col)
...
while (-not $sr.EndOfStream) {
  ...
  if ($part -match  "\[Debug \]" -or $part -match  "\[Debug17 \]") { 
    ...
    if ($part -match  "RPC call started") {
      ...
      $tbRPC.Rows.Add($row)
    } elseif ($part -match  "RPC Log Event") {
      $row.PID = $LP.PID
      $row.TID = $LP.TID
      $line = $sr.ReadLine()
      $verb = (FindSep -FindIn $line -Left "Verb: 	" -Right "SubjectPointer:").Trim()
      if (($verb -eq "START99") -or ($verb -eq "INC43")) {
        $aEvt = $tbRPC.Select("PID = '" + $LP.PID + "' and TID = '" + $LP.TID + "' and SubjectPointer is Null")
        if ($aEvt.Count -gt 0) { 
          $aEvt[0].SubjectPointer = (FindSep -FindIn $line -Left "SubjectPointer: 	" -Right "ObjectPointer:").Trim()
        }
      }
    }
